...
const gulp = require("gulp"),
  ...
  src_type_folder = src_folder + "js3/",
  dist_type_folder = dist_folder + "js3/",
      ...
      plugins: [
        ...
        alias({
          entries: [
            {
              find: "Framework-util",
              replacement: `${src_js2_folder}/util/index.js`,
            },
          ],
        }),
      ],
      ...
      plugins: [
        ...
        alias({
          entries: [
            {
              find: "GC-util",
              replacement: `${src_framework_folder}/util/index.js`,
            },
...
gulp.task("js_typetest", () => {
  console.log('----------------------------------')
  console.log(typescript)
  
  return rollup
    .rollup({
      input: src_type_folder + "index.ts",
      format: "umd",
      plugins: [
        typescript(),
        resolve.nodeResolve(),
    ...
    .then((bundle) => {
      setTimeout(() => {
        gulp.src(src_type_folder + "index.ts").pipe(gulpConnect.reload());
      }, 0);
      return bundle.write({
        file: dist_type_folder + "GCui.js",
        format: "umd",
        name: "GCui",
        sourcemap: true,
      });
...
gulp.task("sprite", function () {
  const spriteData = gulp.src("/images/*.png").pipe(
    spritesmith({
      imgName: "sprite.png",
      cssName: "sprite.css",
    })
...
gulp.task("watch", () => {
  ...
  gulp.watch(src_framework_folder + "**/*.js").on("change", function (done) {
    console.log('src_framework_folder')
    browserSync.reload();
    gulp.series("js_ghilchaekim")(done);
  });

  gulp.watch(src_type_folder + "**/*.ts").on("change", function (done) {
    console.log('dsfsdfsdf')
    browserSync.reload();
    gulp.series("js_typetest")(done);
  });
  ...
  gulp.watch(src_folder + "njk/**/*.+(html|njk)").on("change", function (done) {
    var src = done.replaceAll("\\", "/");
    argv.njk_src = src;
    gulp.series("njk_html")(done);
  });

  gulp.watch("./guide/asset/**/*.njk").on("change", function (done) {
    var src = done.replaceAll("\\", "/");
